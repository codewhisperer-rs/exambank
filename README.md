...
## AI助教与题库系统 Docker 使用指南

这个Docker镜像包含两个集成服务：
1. **AI助教服务**：运行在端口3000
2. **ExamBank题库系统**：运行在端口8000
...
```bash
curl -O https://raw.githubusercontent.com/你的用户名/项目名/main/docker-compose.yml
```
...
3. 访问服务：
   - AI助教：http://localhost:3000
   - ExamBank题库：http://localhost:8000

### 手动启动Docker容器

如果您不想使用Docker Compose，也可以手动启动容器：

```bash
docker run -d \
  --name aqua-ai-exambank \
  -p 3000:3000 -p 8000:8000 \
  -v exambank_data:/app/exambank/data \
  -v exambank_media:/app/exambank/media \
  -v exambank_db:/app/exambank/db \
  aqua-ai-exambank:latest
```

## 数据持久化

所有数据通过Docker卷进行持久化：
- `exambank_data`：题库系统的数据
- `exambank_media`：上传的媒体文件
- `exambank_db`：数据库文件

## 系统配置

需要修改配置，请根据需要挂载自定义配置文件：

```bash
docker run -d \
  --name aqua-ai-exambank \
  -p 3000:3000 -p 8000:8000 \
  -v /path/to/your/config.js:/app/aqua-ai-chat-teacher-assistant/config.js \
  -v exambank_data:/app/exambank/data \
  -v exambank_media:/app/exambank/media \
  -v exambank_db:/app/exambank/db \
  aqua-ai-exambank:latest
...
```bash
docker volume ls
docker volume inspect exambank_data