...
async def ai_grade_comprehensive(request):
    """
    使用大模型API为综合题提供评分（双模型平均）
    返回字段：average_score、score_details，兼容前端展示
    """
    try:
        # 解析请求数据
        data = json.loads(request.body)
        ...
        if not exercise_id or not user_answer:
            return JsonResponse({'status': 'error', 'error': '缺少必要参数'})
        ...
        if exercise.type != 'comprehensive':
            return JsonResponse({'status': 'error', 'error': '该习题不是综合题'})
        ...
        # ===== 调用后端评分（双模型平均）=====
        # 兼容：你的 call_llm_api_for_grading_async 返回的是 dict（average+details）
        # 若还没改成 dict，也做一次 tuple 兼容
        res = await call_llm_api_for_grading_async(question_content, correct_answer, user_answer, model_type)
        ...
        if isinstance(res, dict) and ("average_score" in res or "details" in res):
            # 新结构
            average_score = _to_float(res.get("average_score"))
            details = res.get("details", {}) or res.get("score_details", {}) or {}
            grok_d = details.get("grok", {}) or {}
            deep_d = details.get("deepseek", {}) or {}

            score_details = {
                "grok": {
                    "score": _to_float(grok_d.get("score")),
                    "feedback": (grok_d.get("feedback") or ""),
                    "error": grok_d.get("error")
                },
                "deepseek": {
                    "score": _to_float(deep_d.get("score")),
                    "feedback": (deep_d.get("feedback") or ""),
                    "error": deep_d.get("error")
                }
            }
            # 合并反馈（可选）
            gf = score_details["grok"]["feedback"]
            df = score_details["deepseek"]["feedback"]
            combined_feedback = (
                (f"**Grok反馈**：\n{gf}\n\n" if gf else "") +
                (f"**DeepSeek反馈**：\n{df}" if df else "")
            ).strip()
            ...
            # 兼容老结构: (score, feedback) 视为“单次评分”
            s, fb = res
            s = _to_float(s)
            average_score = s
            score_details = {
                "grok": {"score": s, "feedback": fb, "error": None},
                "deepseek": {"score": None, "feedback": "", "error": "调试阶段未返回"}
            }
            combined_feedback = fb or ""
        else:
            # 其他不明结构
            return JsonResponse({'status': 'error', 'error': f'评分返回结构异常: {type(res)}'})

        # ===== 已登录用户则保存一次尝试记录（用平均分，7 分及以上为基本正确）=====
        ...
        if is_authenticated:
            try:
                # None 保护
                is_correct = bool(average_score is not None and average_score >= 7.0)
        ...
        return JsonResponse({
            'status': 'success',
            'model_used': 'grok+deepseek',
            'average_score': round(average_score, 2) if isinstance(average_score, (int, float)) else None,
            'score_details': score_details,
            'feedback': md_convert(combined_feedback) if combined_feedback else "",
            'correct_answer': md_convert(correct_answer)
        })

    except json.JSONDecodeError:
        return JsonResponse({'status': 'error', 'error': 'JSON格式错误'})
...
async def call_llm_api_for_grading_async(question, correct_answer, user_answer, model_type='grok'):
    """
    异步调用两个大模型（Grok 与 DeepSeek），并返回平均分结果
    保持原函数名与调用方式不变
    返回:
    {
        "average_score": float,
        "details": {
            "grok": {"score": float, "feedback": str},
            "deepseek": {"score": float, "feedback": str}
        }
    }
    """
    try:
        # ====== Grok 模型参数 ======
        grok_url = "https://api.deepseek.com/chat/completions"
        grok_headers = {
            "Authorization": f"Bearer {GROK_API_KEY}",
            "Content-Type": "application/json"
        }
        ...
        async def call_model(api_url, headers, payload, model_type):
            resp = await safe_api_call(api_url, headers, payload, model_type)

            # 兼容：(score, feedback) 的元组返回
            ...
            if isinstance(resp, dict) and resp.get("status") == "success":
                return float(resp.get("score", 0.0)), str(resp.get("feedback", ""))
            ...
            if isinstance(resp, str):
                ...
                if m:
                    return float(m.group(0)), resp
                return 0.0, resp

            raise ValueError(f"{model_type.upper()} 调用失败：{resp}")

        grok_task = asyncio.create_task(call_model(grok_url, grok_headers, grok_payload, "grok"))
        deepseek_task = asyncio.create_task(call_model(deepseek_url, deepseek_headers, deepseek_payload, "deepseek"))

        grok_score, grok_feedback = await grok_task
        deepseek_score, deepseek_feedback = await deepseek_task

        average_score = round((grok_score + deepseek_score) / 2.0, 2)

        return {
            "average_score": average_score,
            "details": {
                "grok": {"score": grok_score, "feedback": grok_feedback},
                "deepseek": {"score": deepseek_score, "feedback": deepseek_feedback}
            }
        }

    except asyncio.TimeoutError:
        error_msg = "双模型API调用超时"
        logger.error(error_msg)
        raise ValueError(error_msg)
    except Exception as e:
        error_msg = f"调用双模型API错误: {str(e)}"
        logger.error(error_msg)
        logger.error(traceback.format_exc())
        raise ValueError(error_msg)
