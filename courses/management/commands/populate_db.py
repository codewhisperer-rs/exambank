...
import json # Import json for options
...
class Command(BaseCommand):
    ...
    # Define the base data directory relative to the project's BASE_DIR
    # Assumes BASE_DIR is set correctly in settings.py to your project root (exambank)
    DATA_DIR = os.path.join(settings.BASE_DIR, 'data')

    # Mapping from directory names to more descriptive book titles if needed
    SUBJECT_MAP = {
        'cn': '计算机网络',
        'co': '计算机组成原理',
        'ds': '数据结构',
    ...
    def process_markdown_images(self, markdown_content, source_file_dir):
        ...
        Assumes images have already been copied to MEDIA_ROOT/images/.

        Args:
            markdown_content (str): The original markdown text.
            source_file_dir (str): The absolute path to the directory containing the original markdown file (for logging context).

        Returns:
            str: Markdown content with image paths replaced by /media/images/ paths.
        """
        if not markdown_content: # Handle empty content
            return ""
            
        # Regex to find ![alt](path "title") or ![alt](path) where path is relative and not http/https
        # It handles optional spaces around the path and optional title part.
        img_pattern = re.compile(r'!\[(.*?)\]\(\s*(?!https?://|/)([^\s\)\"\']+?)(\s+[\"\'].*?[\"\'])?\s*\)')
        updated_markdown = markdown_content
        # Ensure MEDIA_URL ends with a slash
        ...
        for match in img_pattern.finditer(markdown_content):
            ...
            title_part = match.group(3) or '' # This includes the space and quotes if present

            matches_found.append(full_tag)
            # logging.debug(f"    Found potential image tag: {full_tag}, Original Path: '{original_path_str}', Title part: '{title_part}' in dir {source_file_dir}")

            # Extract filename, handle potential path issues
            try:
            ...
            # Reconstruct the title part correctly if it exists
            cleaned_title_part = f' {title_part.strip()}' if title_part.strip() else ''
            new_img_tag = f'![{alt_text}]({new_image_url}{cleaned_title_part})'

            # logging.debug(f"      Replacing '{full_tag}' with '{new_img_tag}'")

            # Use count=1 for replace to avoid issues with identical tags
        ...
        # if matches_found:
        #     logging.debug(f"    Finished processing images in {source_file_dir}. Found {len(matches_found)} tags. Made {len(replacements_made)} replacements.")
        # else:
        #     logging.debug(f"    No relative image tags found in {source_file_dir}.")
    ...
    def extract_answers(self, answers_markdown, source_file_dir):
        ...
        Args:
            answers_markdown (str): 包含答案和解析的 Markdown 文本。
        ...
        if len(parts) == 1: # No type headers found
            content_parts.append(('', parts[0])) # Assume all content belongs to one implicit type
        else:
    ...
    def parse_and_create_exercises(self, section, exercises_markdown, answers_data, source_file_dir):
        """解析习题 Markdown，并结合答案数据创建 Exercise 对象。
        包含简化的选择题选项提取逻辑。

        Args:
            section (Section): 关联的 Section 对象。
            exercises_markdown (str): 包含习题的 Markdown 文本。
            answers_data (dict): 从 extract_answers 返回的字典。
            source_file_dir (str): 源文件目录，用于图片处理。
        """
        logger.info(f"      Starting exercise processing for Section {section.number} ({section.title}) within a transaction.")
        # 同样修改此处正则表达式，支持带有或不带有章节编号和井号的题型标题
        type_pattern = re.compile(r'^(?:#+\s+|[\d\.]+\s+)?(一、单项选择题|二、综合应用题)', re.MULTILINE)
        parts = type_pattern.split(exercises_markdown)

        exercise_order = 1 # Global order counter for exercises within the section

        # If no type headers found, parts will have 1 element (the whole text)
        # If found, parts will be ['', 'Header1', 'Content1', 'Header2', 'Content2', ...]

        content_parts = []
        if len(parts) <= 1:
            # Handle case with no headers, assume implicit type or process based on question format
            ...
            content_parts.append(('', parts[0] if parts else '')) # Use empty header
        else:
